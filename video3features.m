        ...
        if (h*w>2000)
            ...
            if (h*w>2000 && w/h<=1.5 && w/h>=0.3) % 
                ...
                if  size(frame_rear,1)>=60*(vW/768) % 判断车尾后区域达到50（或60）
                    imshow(frame); % 进行标注准备；如不需要标注可删除此行以提升性能
...
for j=1:4
    labelinput = input(['Is the ', Local{j}, ' smoky? ']); % 按左上，左下，下左，下右，输入0表示是无效帧
    if labelinput == 0
        templabel(j) = -1; % 标记坏帧，已经同步标签和特征值向量的指标，因此只标记标签即可去除
%                         Video_xfeatures{j}( frame_order,: ) = -1; % 输入0标记为坏帧
%                         LABEL( frame_order, j ) = -1; % 输入0标记为坏帧
    elseif isempty(labelinput)
            templabel(j) = 0; % 空输入表示非黑烟车，因为黑烟车占大多数
    else 
        templabel(j) = 1;
    end
end
...
elseif isempty(labelinput)
        templabel = 0; % 空输入表示非黑烟车，因为黑烟车占大多数
else 
    templabel = 1;
end
end
